ModifyNetworkUI = function(id, mutations.all, mutations.params)
{
  ...
  shiny::sidebarLayout(
    shiny::sidebarPanel(width = 3, shiny::h3("Mutations"), lapply(mutations.all$mutation, function(mut){shinyBS::bsButton(ns(mut), mutations.all$name[mutations.all$mutation == mut], block = T)})),
    shiny::mainPanel(width = 9,
                     shiny::h1("Modify current network instance"), text_general, shiny::hr(),
                     shiny::h3("Collection"),
                     shiny::verbatimTextOutput(ns("currentCollection"), T),
                     #TODO implement?
                     # shinyBS::bsButton(ns("example"), "See example", shiny::icon("play"), block = T), shiny::br(),
                     shiny::fluidRow(
                       shiny::column(2, shiny::numericInput(ns("iterations"), "Iterations", 1, 0, 10000)),
                       shiny::column(10, shiny::div(shiny::tags$label("for" = ns("set_probs"), "Set probabilities"), shinyBS::bsButton(ns("set_probs"), "Uniform probabilities", type = "toggle", block = T)))),
                     shinyBS::bsButton(ns("mutate"), " View network", shiny::icon("play", lib = "glyphicon"), block = T),
